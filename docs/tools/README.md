...
| `media` | 10 | **S94.** Sovereign media-stack orchestration domain (Radarr/Sonarr/Prowlarr/qtor/Plex/<media-service>/TMDb) — vault(env)-backed service clients, a config-status tool, the read/search surface (`media_search`, `media_status`), (MEDIA-03) `media_request` — tiered-confirmation add/request driving Radarr/Sonarr → the download client, (MEDIA-04) `media_organize`/`media_delete`/`media_cleanup` — non-destructive organize plus hard-typed-confirmation destructive delete/bulk cleanup, and (MEDIA-05) `media_recommend`/`media_on_deck`/`media_recently_added` — stateless watch-history-driven recommendations + engagement surface. Taste-memory personalization + Lumina surface wiring land in MEDIA-06..07. | [`personal-life/media.md`](personal-life/media.md) |
