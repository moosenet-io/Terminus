...
| `cortex` | ~10 (3 live + 7 deprecated aliases) | **CXEG-01: SSH-relay era retired**, re-scaffolded Atlas-backed. `cortex_scope`/`cortex_review`/`cortex_audit` are pending-rebuild stubs (CXEG-02/04/11); the other 7 names are zero-I/O deprecation aliases pointing at the Atlas KG's `kg_*` tools. | [`code-git/cortex.md`](code-git/cortex.md) |
