...
| `media` | 4 | **S94.** Sovereign media-stack orchestration domain (Radarr/Sonarr/Prowlarr/qtor/Plex/<media-service>/TMDb) — one thin, env-config-backed client per service, `media_domain_status`, `media_search` (fuzzy TMDb title resolution), `media_status` (cross-service presence/quality), and (MEDIA-03) `media_request` — tiered-confirmation add/request (Radarr/Sonarr → download client, optional <media-service> tracking). Organize/recommend tools land in MEDIA-04..07. | [`media.md`](media.md) |
