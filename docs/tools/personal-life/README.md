...
| `media` | 7 | **S94.** Sovereign media-stack orchestration domain (Radarr/Sonarr/Prowlarr/qtor/Plex/<media-service>/TMDb) — one thin, env-config-backed client per service, `media_domain_status`, `media_search` (fuzzy TMDb title resolution), `media_status` (cross-service presence/quality), (MEDIA-03) `media_request` — tiered-confirmation add/request (Radarr/Sonarr → download client, optional <media-service> tracking), and (MEDIA-04) `media_organize`/`media_delete`/`media_cleanup` — non-destructive organize (reuses MEDIA-03 tiering) plus hard-typed-confirmation destructive delete/bulk cleanup. Recommend tools land in MEDIA-05..07. | [`media.md`](media.md) |
