...
fn pii_gate_blocks_infra_names_and_secrets() {
    assert!(scan_pii("the node at 10.0.0.103 hosts it").is_some());
    assert!(scan_pii("see CT305 for the orchestrator").is_some());
    assert!(scan_pii("email user@example.com").is_some());
    assert!(scan_pii("Authorization: Bearer abc123").is_some());
    assert!(scan_pii("the operator Peter prefers direct feedback").is_some());
...
fn corpus_loader_rejects_pii_taint() {
    let tainted = r#"{
        "name":"bad","k":2,
        "docs":[{"id":"d1","text":"reach the box at 10.0.0.5 now"}],
