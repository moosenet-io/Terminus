#!/usr/bin/env bash
# Validator for java-blitz-j1 (Validate.validateEmail). Stage contract per the
# corpus README: STAGE:COMPILE / STAGE:TESTS / STAGE:CHANGE, TOOLCHAIN:missing.
set -uo pipefail
WORK="${MINT_WORK:-$PWD}"
...
# ---- COMPILE: all workspace sources (baseline + model output) ----
...
# ---- TESTS: baseline API still behaves (regression guard) ----
