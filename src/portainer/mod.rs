...
//!
//! Four tools mirroring the Python portainer_tools.py on CT322 exactly:
...
//!
//! Required env vars:
//!   PORTAINER_URL        — e.g. https://portainer.example:9443 (self-signed cert)
...
impl PortainerConfig {
    ...
    async fn api_get(
        ...
        let resp = client
            ...
            .await
            .map_err(|e| {
                warn!("portainer: request to {url} failed: {e}");
                ToolError::Http("The container service (Portainer) is unreachable.".into())
            })?;

        let status = resp.status();
        let raw = resp.text().await.map_err(|e| {
            warn!("portainer: reading response from {url} failed: {e}");
            ToolError::Http("The container service (Portainer) is unreachable.".into())
        })?;
