...
///
/// `alias_pairs` has already discarded any cyclic component, so a cycle cannot
/// normally reach here. The revisit guard remains as a terminating backstop for
/// a table built by other means (it is exercised by the unit tests, which call
/// `collapse` directly): it stops, and it does NOT invent a canonical key.
...
fn alias_pairs() -> Vec<(String, String)> {
    ...
    if let Ok(raw) = std::env::var(ALIASES_ENV) {
        for (alias, canonical) in parse_alias_env(&raw) {
            // The built-in equivalence classes are IMMUTABLE from configuration.
            //
            // Review finding (codex, verified): letting an env entry re-point a
            // built-in let one line of ops config re-split a project —
            // `harmony=harmony-x` gave resolve("HARMONY") = "harmony-x" while
            // resolve("HARM") = "harm", two live keys for one project — and
            // `chord=harmony` could MERGE two unrelated projects onto one graph.
            // That is the very state this type exists to make unrepresentable,
            // so config may ADD mappings (a project UUID) but never redefine a
            // built-in one. Changing a canonical identity is a code change, and
            // goes through review like any other.
            if is_builtin_key(&alias) {
                continue;
            }
    ...
    drop_cyclic_components(pairs)
}

/// Whether `key` appears in the built-in table on EITHER side — as an alias or
/// as a canonical target. Both are protected: re-pointing `chord` re-splits the
/// class, and re-pointing `chrd` moves the canonical out from under it.
...
/// Drop every entry that participates in a cycle.
///
/// Review finding (codex, verified) — and it REVERSES a round-1 decision, so
/// the reasoning is recorded. Round 1 correctly objected that a cycle which
/// terminated at "the key I re-entered on" resolved `a=b,b=c,c=a` to a->a,
/// b->b, c->c: one alias group, three store keys. I then made cycle members
/// converge on the lexicographically smallest member. Round 3 objected that
/// this INVENTS an identity — it silently merges projects that may be unrelated
/// onto whichever key happens to sort first, which is data loss.
///
/// Both objections are right about their own failure. The tie-break is which
/// failure is worse: silently merging distinct projects corrupts data, while
/// refusing to group them leaves the status quo and tells the truth. A cycle
/// expresses no canonical member, so there is nothing to honestly pick. The
/// entries are therefore DISCARDED — each key resolves to itself via
/// normalization — and the invalid configuration is surfaced by
/// [`invalid_alias_entries`] rather than guessed at.
fn drop_cyclic_components(pairs: Vec<(String, String)>) -> Vec<(String, String)> {
    let mut poisoned: BTreeSet<String> = BTreeSet::new();
    for (start, _) in &pairs {
        let mut walk: Vec<String> = Vec::new();
        let mut cur = start.clone();
        loop {
            if let Some(i) = walk.iter().position(|s| *s == cur) {
                poisoned.extend(walk[i..].iter().cloned());
                break;
            }
            walk.push(cur.clone());
            match pairs.iter().find(|(a, _)| *a == cur) {
    ...
    pairs
        .into_iter()
        .filter(|(a, _)| !poisoned.contains(a))
        .collect()
}

/// Config entries that were REJECTED, with why — so an operator can see that a
/// line of `SCRIBE_KG_PROJECT_ALIASES` is not doing what they think. Silently
/// ignoring a bad entry would be its own absence-that-looks-like-a-result.
pub fn invalid_alias_entries() -> Vec<String> {
    let Ok(raw) = std::env::var(ALIASES_ENV) else {
        return Vec::new();
    };
    let parsed = parse_alias_env(&raw);
    let mut out = Vec::new();
    for (alias, canonical) in &parsed {
        if is_builtin_key(alias) {
            out.push(format!(
                "{alias}={canonical}: rejected — '{alias}' is a built-in key and built-in \
                 equivalence classes cannot be redefined by configuration"
            ));
        }
    }
    let kept = alias_pairs();
    for (alias, canonical) in &parsed {
        if !is_builtin_key(alias) && !kept.iter().any(|(a, _)| a == alias) {
            out.push(format!(
                "{alias}={canonical}: rejected — part of a cycle; a cycle names no canonical key, \
                 so the whole component is discarded rather than guessed at"
            ));
        }
    }
    out
...
/// away entirely is SKIPPED. Both sides are normalized so an operator can write
/// `Chord=CHRD` and get the same table as `chord=chrd`.
///
/// A self-mapping (`x=x`) is deliberately NOT special-cased here: it is a
/// 1-cycle, and `drop_cyclic_components` already discards it by the same rule
/// that discards any other cycle. A dedicated skip was redundant once cycles
/// were dropped — a mutant proved no test could distinguish it — and it also
/// suppressed the entry from `invalid_alias_entries`, hiding a bad config line
/// from the operator who wrote it.
...
mod tests {
    ...
    #[test]
    fn env_table_adds_a_uuid_but_cannot_redefine_a_builtin() {
        let _g = env_lock();
        std::env::set_var( // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
            ALIASES_ENV,
            "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee=chrd, harmony=harmony-x, chrd=somewhere", // pii-test-fixture (fabricated uuid shape)
        );
        // ADDING a mapping (the real use case: a Plane project UUID) works.
        assert_eq!(
        ...
        // REDEFINING a built-in must NOT work, from either side. Allowing it let
        // one config line re-split a project (HARMONY -> harmony-x while
        // HARM -> harm) or merge two unrelated ones.
        assert_eq!(
            ProjectKey::resolve("HARMONY").as_str(),
            "harm",
            "a built-in alias must not be redefinable by config"
        );
        assert_eq!(
            ProjectKey::resolve("chrd").as_str(),
            "chrd",
            "a built-in canonical must not be re-pointed by config"
        );
        // And the rejection is REPORTED, not silent.
        let bad = invalid_alias_entries();
        assert!(bad.iter().any(|e| e.starts_with("harmony=")), "{bad:?}");
        assert!(bad.iter().any(|e| e.starts_with("chrd=")), "{bad:?}");
    ...
    #[test]
    fn a_merging_config_cannot_point_one_builtin_class_at_another() {
        let _g = env_lock();
        // `chord=harmony` would merge Chord's graph onto Harmony's.
        std::env::set_var(ALIASES_ENV, "chord=harmony"); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
        assert_eq!(ProjectKey::resolve("chord").as_str(), "chrd", "must stay in its own class");
        assert_eq!(ProjectKey::resolve("harmony").as_str(), "harm");
        std::env::remove_var(ALIASES_ENV); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
    }
    ...
    #[test]
    fn collapse_terminates_on_a_cyclic_table_reached_by_any_other_route() {
        // `alias_pairs` discards cycles, so this cannot arrive through config —
        // but `collapse` must still be non-looping on its own terms, because a
        // hang in key resolution takes down every kg_* read. Exercised directly
        // rather than left as an unreachable, untestable backstop (a mutant
        // showed the public API could no longer distinguish it).
        let cyclic: Vec<(String, String)> = [("a", "b"), ("b", "c"), ("c", "a")]
            .iter()
            .map(|(x, y)| (x.to_string(), y.to_string()))
            .collect();
        let got = collapse("a".to_string(), &cyclic);
        assert!(["a", "b", "c"].contains(&got.as_str()), "terminates, got {got}");
        // Deterministic, and it does not invent a key outside the cycle.
        assert_eq!(collapse("a".to_string(), &cyclic), got);

        // A self-mapping is a 1-cycle and must also terminate.
        let selfmap = vec![("x".to_string(), "x".to_string())];
        assert_eq!(collapse("x".to_string(), &selfmap), "x");
    }

    #[test]
    fn a_self_mapping_is_discarded_as_a_cycle_and_reported() {
        let _g = env_lock();
        // Not a built-in, so it reaches the cycle logic rather than the
        // built-in-immutability guard.
        std::env::set_var(ALIASES_ENV, "solo=solo"); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
        assert_eq!(ProjectKey::resolve("solo").as_str(), "solo", "inert");
        let bad = invalid_alias_entries();
        assert!(
            bad.iter().any(|e| e.starts_with("solo=") && e.contains("cycle")),
            "a self-mapping must be reported, not silently swallowed: {bad:?}"
        );
        std::env::remove_var(ALIASES_ENV); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
    }
    ...
    fn a_cyclic_alias_config_terminates() {
        ...
        // A cycle names no canonical member, so the whole component is
        // DISCARDED — every key resolves to itself, exactly as if the entries
        // were absent. It must not merge them onto an invented winner (that
        // would silently fold together projects that may be unrelated), and it
        // must not hang.
        assert_eq!(ProjectKey::resolve("a").as_str(), "a");
        assert_eq!(ProjectKey::resolve("b").as_str(), "b");
        assert_eq!(ProjectKey::resolve("c").as_str(), "c");
        // Deterministic across calls, so it cannot flap between two stores.
        assert_eq!(ProjectKey::resolve("a").as_str(), "a");
        // The invalid config is REPORTED rather than silently ignored.
        let bad = invalid_alias_entries();
        assert_eq!(bad.len(), 3, "every cyclic entry must be reported: {bad:?}");
        assert!(bad.iter().all(|e| e.contains("cycle")), "{bad:?}");
        // A non-cyclic entry alongside a cycle still works.
        std::env::set_var(ALIASES_ENV, "a=b,b=c,c=a,ok=term"); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
        assert_eq!(ProjectKey::resolve("ok").as_str(), "term", "good entries survive a bad one");
        assert_eq!(ProjectKey::resolve("a").as_str(), "a");
        std::env::remove_var(ALIASES_ENV); // hermeticity-allow: serialized on env_lock(); serial_test is unavailable to the standalone harness
    }
