...
//! (default `/opt/chord/intake-corpus`). For each selected case it:
...
//!
//! Toolchain reality: pvf1 has python3/g++/bash/sqlite3 but may lack cargo/node.
...
/// Default corpus location on pvf1; overridable via `INTAKE_CORPUS_DIR`.
const DEFAULT_CORPUS_DIR: &str = "/opt/chord/intake-corpus";
...
/// runnable with bash/python/g++/sqlite3 (assumed present on pvf1).
