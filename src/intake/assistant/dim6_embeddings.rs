...
//! pattern (private IPs, container ids like `CT212`, email addresses, names in a
...
///   - container ids `CT` followed by a digit (e.g. `CT212`),
...
pub fn scan_pii(text: &str) -> Option<String> {
    ...
    const NAME_BLOCKLIST: &[&str] = &[
        "peter", "moose", "lumina", "ironclaw", "axon", "vigil", "sentinel",
    ];
...
mod tests {
    ...
    fn pii_gate_rejects_infra_and_pii() {
        assert!(scan_pii("the host at 10.0.0.98 runs the service").is_some());
        assert!(scan_pii("see container CT212 for details").is_some());
        assert!(scan_pii("mail me at someone@example.com").is_some());
        assert!(scan_pii("api_key=abc").is_some());
        assert!(scan_pii("token=AKIA1234567890ABCDEF1234").is_some());
        ...
        assert!(scan_pii("ask Peter about it").is_some());
    ...
    fn corpus_from_json_rejects_pii_doc() {
        let bad = r#"{
            "name":"x","k":2,
            "docs":[{"id":"d1","text":"reach the box at 10.0.0.5"}],
