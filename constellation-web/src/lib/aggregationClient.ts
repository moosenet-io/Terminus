...
/** CONST-27 will add a real `role` claim (§3.4); CONST-20 adds the field ahead of that item
 *  landing (see `hooks/useAuthRole.ts`'s seam note) so the mock adapter and any real backend
 *  that already sends it (or doesn't, yet) both type-check without a breaking change later. */
export type AuthRole = 'viewer' | 'operator';

export interface AuthMeResponse {
  authenticated: boolean;
  username: string | null;
  /** Absent until CONST-27 lands server-side; treat as unknown, not 'viewer', when absent
   *  (see `useAuthRole` for the fallback policy). */
  role?: AuthRole;
}
